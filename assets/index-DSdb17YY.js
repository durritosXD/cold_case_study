...
`,h="AIzaSyAfm0MxLtOyUCioIt7na0RZ45XtLhQX3Y0";async function b(e){var l;const t=`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${h}`,n=await fetch(t,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:v(e)}]}],generationConfig:{responseMimeType:"application/json",temperature:.3,maxOutputTokens:8192}})});if(!n.ok){const d=await n.json().catch(()=>({}));throw new Error(((l=d==null?void 0:d.error)==null?void 0:l.message)||`HTTP ${n.status}`)}const i=(await n.json()).candidates[0];if(i.finishReason==="MAX_TOKENS")throw new Error("Case analysis was too detailed and got cut off (MAX_TOKENS). Try reducing the case text or asking for a shorter summary.");const o=i.content.parts[0].text,c=o.replace(/```json|```/g,"").trim();try{return JSON.parse(c)}catch(d){throw console.error("Raw response:",o),new Error(`Invalid JSON returned from AI: ${d.message}`)}}function r(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function p(e){return e==="high"?"var(--danger)":e==="medium"?"var(--warning)":"var(--muted)"}function E(e){return e==="critical"?"critical":e==="moderate"?"moderate":"minor"}function w(e){return e==="strong"?"badge-strong":e==="weak"?"badge-weak":"badge-missing"}function g(e){return e>70?"var(--danger)":e>=40?"var(--warning)":"var(--success)"}function I(e){const t=document.getElementById("summary-text");t&&(t.textContent=e.case_summary),a("section-summary")}function x(e){const t=document.getElementById("timeline-entries");!t||!e.timeline||(t.innerHTML=e.timeline.map(n=>`
  ...
  `).join(""),a("section-suspects")}function L(e){const t=document.getElementById("inconsistencies-list");!t||!e.inconsistencies||(t.innerHTML=e.inconsistencies.map(n=>`
    <div class="alert-card ${E(n.severity)}">
      <span class="severity-badge">${n.severity.toUpperCase()}</span>
      <p>${r(n.description)}</p>
    </div>
  `).join(""),a("section-inconsistencies"))}function _(e){const t=document.getElementById("evidence-grid");!t||!e.evidence||(t.innerHTML=e.evidence.map(n=>`
  ...
  `).join(""),a("section-evidence"))}function B(e){const t=document.getElementById("missing-list");!t||!e.missing_info||(t.innerHTML=e.missing_info.map(n=>`<li>${r(n)}</li>`).join(""),a("section-missing"))}function C(e){const t=e.verdict;if(!t)return;const n=Math.min(100,Math.max(0,t.confidence_percent)),s=n>=70?"var(--danger)":n>=40?"var(--warning)":"var(--success)",i=document.getElementById("verdict-scenario"),o=document.getElementById("verdict-pct"),c=document.getElementById("verdict-bar-fill"),l=document.getElementById("verdict-reasoning");i&&(i.textContent=t.most_likely_scenario),o&&(o.textContent=n+"%"),c&&(c.style.cssText=`width:${n}%; background:${s}`),l&&(l.textContent=t.reasoning),a("section-verdict")}function A(e){const t=document.getElementById("geo-text");t&&e.geographic_profile&&(t.textContent=e.geographic_profile,a("section-geo"))}function T(e){const t=document.getElementById("next-steps-list");!t||!e.investigative_next_steps||(t.innerHTML="<ul>"+e.investigative_next_steps.map(n=>`
  ...
  `).join("")+"</ul>",a("section-nextsteps"))}async function S(e){const t=n=>new Promise(s=>setTimeout(s,n));I(e),await t(400),$(e),await t(600),x(e),await t(400),L(e),await t(300),_(e),await t(300),A(e),await t(300),T(e),await t(300),B(e),await t(300),C(e),document.getElementById("results").scrollIntoView({behavior:"smooth"})}function a(e){const t=document.getElementById(e);t&&(t.style.display="",t.classList.add("fade-in"))}function y(){document.querySelectorAll(".result-section").forEach(e=>{e.style.display="none",e.classList.remove("fade-in")})}function f(e){const t=document.getElementById("analyse-btn"),n=document.getElementById("loading");t.disabled=e,t.textContent=e?"ANALYSING...":"ANALYSE CASE",n.style.display=e?"flex":"none"}function u(e){const t=document.getElementById("error-box");t.innerHTML=e,t.style.display="block"}function k(){const e=document.getElementById("error-box");e.innerHTML="",e.style.display="none"}async function m(e){try{const t=await fetch(`cases/${e}`);if(!t.ok)throw new Error(`Could not load ${e}`);const n=await t.text();document.getElementById("case-input").value=n}catch(t){u(`Failed to load preset: ${t.message}`)}}function O(e){const t=e.target.files[0];if(!t)return;const n=new FileReader;n.onload=s=>{document.getElementById("case-input").value=s.target.result},n.readAsText(t)}async function M(){const e=document.getElementById("case-input").value.trim();if(!e){u("Please paste a case description or upload a .txt file.");return}if(e.length<100){u("Case text too short. Provide at least a paragraph of detail.");return}k(),y(),f(!0);try{const t=await b(e);S(t)}catch(t){let n=`Analysis failed: ${t.message}.`;(t.message.toLocaleLowerCase().includes("leaked")||t.message.includes("401")||t.message.includes("403"))&&(n=`<strong>ACCESS REVOKED:</strong> API Key leaked or invalid. <br><br> 
             1. Go to <a href="https://aistudio.google.com/" target="_blank">Google AI Studio</a> <br>
             2. Generate a new key <br>
             3. Update your <code>.env</code> file.`),u(n),console.error(t)}finally{f(!1)}}document.addEventListener("DOMContentLoaded",()=>{document.getElementById("analyse-btn").addEventListener("click",M),document.getElementById("file-upload").addEventListener("change",O),document.getElementById("upload-btn").addEventListener("click",()=>{document.getElementById("file-upload").click()}),document.getElementById("btn-cooper").addEventListener("click",()=>m("db-cooper.txt")),document.getElementById("btn-zodiac").addEventListener("click",()=>m("zodiac.txt")),document.getElementById("btn-dahlia").addEventListener("click",()=>m("black-dahlia.txt")),y()});
